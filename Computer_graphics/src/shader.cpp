...
void Shader::setUniform(std::string name, glmath::vec3 vec) {
    GLCall(glUniform3fv(GetUniformLocation(name), 1, vec.value_ptr()));
}


void Shader::setUniform(std::string name, glmath::mat4 transform) {
    GLCall(glUniformMatrix4fv(GetUniformLocation(name), 1, false, transform.value_ptr()));
}
